...
/*                                                                           */
/*  Hypertext instructions for GTriangle are available on the Web at          */
/*                                                                           */
...
/*                                                                           */
/*  Three papers discussing aspects of GTriangle are available.  A short      */
/*    overview appears in "GTriangle:  Engineering a 2D Quality Mesh          */
...
/*                                                                           */
/*  GTriangle was created as part of the Quake Project in the School of       */
...
/*                                                                           */
/*  GTriangle's Delaunay refinement algorithm for quality mesh generation is  */
...
/*    GTriangle using the quad-edge data structure, but the switch to a       */
/*    triangle-based data structure sped GTriangle by a factor of two.)  The  */
...
/*    Information Science 9(3):219-242, 1980.  GTriangle's improvement of the */
...
/*    common case, where t is the number of triangles_ that intersect the     */
...
/*                                                                           */
/*  Ruppert's Delaunay refinement algorithm typically generates triangles_    */
...
/*   meshes you can generate with GTriangle.  It also reduces the likelihood  */
...
/* To compile GTriangle as a callable object library (triangle.o), define the */
/*   TRILIBRARY symbol.  Read the file triangle.h for details on how to call */
/*   the procedure triangulate() that results.                               */

/* #define TRILIBRARY */

/* It is possible to generate a smaller version of GTriangle using one or     */
...
/*   this solution might slow GTriangle down quite a bit.  To use volatile    */
...
#define TRIPERBLOCK 4092           /* Number of triangles_ allocated at once. */
#define SUBSEGPERBLOCK 508       /* Number of subsegments allocated at once. */
#define VERTEXPERBLOCK 4092         /* Number of vertices allocated at once. */
#define VIRUSPERBLOCK 1020   /* Number of virus triangles_ allocated at once. */
/* Number of encroached subsegments allocated at once. */
#define BADSUBSEGPERBLOCK 252
/* Number of skinny triangles_ allocated at once. */
#define BADTRIPERBLOCK 4092
/* Number of flipped triangles_ allocated at once. */
...
/*   how large a random sample of triangles_ to inspect.                      */
...
#endif /* LINUX */
#ifdef TRILIBRARY
#include "edgevis/libs/triangle/triangle.h"
#endif /* TRILIBRARY */
...
/*                                                                           */
/*  There are three:  vertices, triangles_, and subsegments (abbreviated      */
...
/*  defined by the user, that triangles_ may not lie across.                  */
/*                                                                           */
/*  A triangle consists of a list of three vertices, a list of three         */
/*  adjoining triangles_, a list of three adjoining subsegments (when         */
...
/*  triangles_ may not be present.  For a triangle in the interior of the     */
/*  mesh, often no neighboring subsegments are present.  Such absent         */
/*  triangles_ and subsegments are never represented by NULL pointers; they   */
...
/*  information as well.  The pointers to adjoining vertices, triangles_, and */
...
/*  sandwiched between two triangles_, or resting against one triangle on an  */
...
/*  two adjoining subsegments, and a list of two adjoining triangles_.  One   */
/*  of the two adjoining triangles_ may not be present (though there should   */
...
/*                                                                           */
/*  Like triangles_, subsegments maintain information about the relative      */
...
/*  updated except during this phase.  GEdge flipping during segment          */
...
/*  about what triangles_, subfacets, or subsegments they are linked to.      */
/*                                                                           */
/*****************************************************************************/
...
/*                                                                           */
/*  Oriented triangles_ and oriented subsegments will usually be referred to  */
/*  as "handles."  A handle is essentially a pointer into the mesh; it       */
/*  allows you to "hold" one particular part of the mesh.  Handles are used  */
/*  to specify the regions in which one is traversing and modifying the mesh.*/
/*  A single `triangle' may be held by many handles, or none at all.  (The   */
/*  latter case is not a memory leak, because the triangle is still          */
/*  connected to other triangles_ in the mesh.)                               */
/*                                                                           */
...
/*  important.  For instance, when two triangles_ are glued together by the   */
/*  bond() primitive, they are glued at the edges on which the handles lie.  */
/*                                                                           */
/*  Because vertices have no information about which triangles_ they are      */
...
/*   adjoining triangles_, plus three pointers to vertices, plus three        */
...
/*   pointers to adjoining triangles_, plus one boundary marker, plus one     */
/*   segment number.                                                         */
...
/* A queue used to store bad triangles_.  The key is the square of the cosine */
...
/* A stack of triangles_ flipped during the most recent vertex insertion.     */
...
/* Mesh data structure.  GTriangle operates on only one mesh, but the mesh    */
/*   structure is used (instead of global variables) to allow reentrancy.    */

struct mesh {

/* Variables used to allocate memory for triangles_, subsegments, vertices,   */
/*   viri (triangles_ being eaten), encroached segments, bad (skinny or too   */
/*   large) triangles_, and splay tree nodes.                                 */
...
/* Variable that maintains the stack of recently flipped triangles_.          */
...
/* Other variables. */
  ...
  int inelements;                              /* Number of input triangles_. */
  ...
  int vertexmarkindex;         /* GIndex to find boundary marker of a vertex. */
  int vertex2triindex;     /* GIndex to find a triangle adjacent to a vertex. */
  int highorderindex;  /* GIndex to find extra nodes for high-order elements. */
  int elemattribindex;            /* GIndex to find attributes of a triangle. */
  int areaboundindex;             /* GIndex to find area bound of a triangle. */
...
/*   nonodewritten: -paramN switch.  noelewritten: -E switch.                     */
...
/*  other triangles_, with orientations.  Each pointer points not to the      */
/*  first byte of a triangle, but to one of the first three bytes of a       */
/*  triangle.  It is necessary to extract both the triangle itself and the   */
/*  orientation.  To save memory, I keep both pieces of information in one   */
/*  pointer.  To make this possible, I assume that all triangles_ are aligned */
/*  to four-byte boundaries.  The decode() routine below decodes a pointer,  */
/*  extracting an orientation (in the range 0 to 2) and a pointer to the     */
/*  beginning of a triangle.  The encode() routine compresses a pointer to a */
/*  triangle and an orientation into a single pointer.  My assumptions that  */
/*  triangles_ are four-byte-aligned and that the `unsigned long' type is     */
...
/*                                                                           */
/*  Below, triangles_ and subsegments are denoted by their vertices.  The     */
...
/*                                                                           */
/*  bond:  Bond two triangles_ together at the resepective handles.           */
/*  bond(abc, bad)                                                           */
/*                                                                           */
/*                                                                           */
/*  For subsegments:                                                         */
...
/*   relies on the assumption that all triangles_ are aligned to four-byte    */
...
/* Test for equality of oriented triangles_.                                  */
...
/********* Primitives for interacting triangles_ and subsegments      *********/
/*                                                                           */
/*                                                                           */
...
#endif /* not CDT_ONLY */
  printf(
    "    -A  Applies attributes to identify triangles_ in certain regions.\n");
  printf("    -c  Encloses the convex hull with segments.\n");
#ifndef CDT_ONLY
  printf("    -D  Conforming Delaunay:  all triangles_ are truly Delaunay.\n");
#endif /* not CDT_ONLY */
/*
...
*/
  ...
  printf("    -paramN  Suppresses output of .node file.\n");
...
#endif /* not REDUCED */
  printf("    -ls  Uses vertical cuts only, rather than alternating cuts.\n");
#ifndef REDUCED
#ifndef CDT_ONLY
  printf(
  ...
  printf("    -h  Help:  Detailed instructions for GTriangle.\n");
  triexit(0);
}
...
#endif /* not SINGLE */
  printf(
"GTriangle generates exact Delaunay triangulations, constrained Delaunay\n");
  printf(
...
"        Delaunay (not just constrained Delaunay) triangulation, use -D as\n");
  printf(
"        well.  When -p is not used, GTriangle reads a .node file by default.\n"
);
  printf(
...
"        (180 - 2 theta) on the largest angle.  If the minimum angle is 28.6\n"
  ...
  printf(
"        degrees or smaller, GTriangle is mathematically guaranteed to\n");
  printf(
"        terminate (assuming infinite precision arithmetic--GTriangle may\n");
  printf(
"        fail to terminate if you run out of precision).  In practice,\n");
  printf(
"        GTriangle often succeeds for minimum angles up to 34 degrees.  For\n");
  printf(
...
"        procedure in triangle.c to encode any constraint you like, then\n");
  printf(
"        recompile GTriangle.  The other is to compile triangle.c with the\n");
  printf(
"        EXTERNAL_TEST symbol set (compiler switch -DEXTERNAL_TEST), then\n");
  printf(
"        link GTriangle with a separate object file that implements\n");
  printf(
...
"        to.  Attributes are assigned to regions by the .poly file.  If a\n");
  printf(
"        region is not explicitly marked by the .poly file, triangles_ in\n");
  printf(
...
"        segments the PSLG has.  If you do not use this switch when\n");
  printf(
"        triangulating a PSLG, GTriangle assumes that you have identified the\n"
);
  printf(
...
"        and a convex hull segment, which can cause overrefinement (and\n");
  printf(
"        possibly failure if GTriangle runs out of precision).  If you are\n");
  printf(
...
"    -D  Conforming Delaunay triangulation:  use this switch if you want to\n"
  ...
  printf(
"        ensure that all the triangles_ in the mesh are Delaunay, and not\n");
  printf(
...
"        circle is encroached.  It usually increases the number of vertices\n"
  ...
  printf("        and triangles_.\n");
  printf(
"    -j  Jettisons vertices that are not part of the final triangulation\n");
  printf(
"        from the output .node file.  By default, GTriangle copies all\n");
  printf(
...
"        may be duplicated.  See the discussion of Voronoi diagrams below.\n");
  printf(
"    -n  Outputs (to a .neigh file) a list of triangles_ neighboring each\n");
  printf("        triangle.\n");
  printf(
...
"        to maintain constraining segments on later refinements of the mesh.\n"
  ...
  printf("    -paramN  No output .node file.\n");
...
"        using a .node file for input, because no .node file is written, so\n"
  ...
  printf(
"    -X  No exact arithmetic.  Normally, GTriangle uses exact floating-point\n"
);
  printf(
...
"        Disabling exact arithmetic with the -X switch causes a small\n");
  printf(
"        improvement in speed and creates the possibility that GTriangle will\n"
);
...
"        first vertex of the input .node or .poly file.  However, this\n");
  printf(
"        switch is useful when calling GTriangle from another program.\n");
  printf(
...
"        adjacent mesh.  Be forewarned that you will probably sacrifice much\n"
  ...
  printf(
"        of the quality of the mesh; GTriangle will try, but the resulting\n");
  printf(
"        mesh may contain poorly shaped triangles_.  Works well if all the\n");
  printf(
...
"        number.  If you specify this switch with no number after it,\n");
  printf(
"        the limit is set to zero.  GTriangle always adds vertices at segment\n"
);
  printf(
"        intersections, even if it needs to use more vertices than the limit\n"
);
  printf(
"        you set.  When GTriangle inserts segments by splitting (-s), it\n");
  printf(
...
"        triangulation.  Warning:  does not use exact arithmetic for all\n");
  ...
  printf(
"    -ls  Uses only vertical cuts in the divide-and-conquer algorithm.  By\n");
  printf(
"        default, GTriangle alternates between vertical and horizontal cuts,\n"
);
  printf(
...
"        is true to Ruppert's original algorithm, but can create needlessly\n"
  ...
  printf(
"        small triangles_.  This switch is primarily of theoretical interest.\n"
);
  printf(
...
"        checking, even if the -X switch is used.  Useful if you suspect\n");
  printf("        GTriangle is buggy.\n");
  printf(
"    -Q  Quiet:  Suppresses all explanation of what GTriangle is doing,\n");
  printf("        unless an error occurs.\n");
  printf(
"    -V  Verbose:  Gives detailed information about what GTriangle is doing.\n"
);
  printf(
...
"        detailed statistics.  `-VV' gives vertex-by-vertex details, and\n");
  printf(
"        prints so much that GTriangle runs much more slowly.  `-VVVV' gives\n"
);
...
"  of the CDT.  (A constrained Delaunay triangulation is not truly a\n");
  printf(
"  Delaunay triangulation, because some of its triangles_ might not be\n");
  printf(
...
"  A conforming constrained Delaunay triangulation (CCDT) of a PSLG is a\n");
  printf(
"  triangulation of a PSLG whose triangles_ are constrained Delaunay.  New\n");
...
"  subsegments, but not to guarantee that segments are respected; rather, to\n"
  ...
  printf(
"  improve the quality of the triangles_.  The high-quality meshes produced\n");
  printf(
...
"  All files may contain comments prefixed by the character '#'.  Vertices,\n"
  ...
  printf(
"  triangles_, edges, holes, and maximum area constraints must be numbered\n");
  printf(
"  consecutively, starting from either 1 or 0.  Whichever you choose, all\n");
  printf(
"  input files must be consistent; if the vertices are numbered from 1, so\n");
  printf(
"  must be all other objects.  GTriangle automatically detects your choice\n");
  printf(
"  while reading the .node (or .poly) file.  (When calling GTriangle from\n");
  printf(
...
"    PSLG segments; a complete description appears in a section below.  The\n"
  ...
  printf(
"    .node file produced by GTriangle contains boundary markers in the last\n");
  printf("    column unless they are suppressed by the -B switch.\n\n");
  printf("  .ele files:\n");
  printf(
"    First line:  <# of triangles_> <nodes per triangle> <# of attributes>\n");
  printf(
...
"    The attributes are just like those of .node files.  Because there is no\n"
  ...
  printf(
"    simple mapping from input to output triangles_, GTriangle attempts to\n");
  printf(
"    interpolate attributes, and may cause a lot of diffusion of attributes\n"
);
  printf(
"    among nearby triangles_ as the triangulation is refined.  Attributes do\n"
);
  printf("    not diffuse across segments, so attributes used to identify\n");
  printf("    segment-bounded regions remain intact.\n\n");
  printf(
"    In .ele files produced by GTriangle, each triangular element has three\n");
  printf(
...
"    that the vertices are listed in a separate .node file; .poly files\n");
  printf(
"    produced by GTriangle always have this format.  A vertex set represented\n"
);
  printf(
...
"    means that you must include its endpoints in the vertex list.  Each\n");
  ...
  printf(
"    If -q, -a, -u, and -s are not selected, GTriangle produces a constrained\n"
);
  printf(
"    Delaunay triangulation (CDT), in which each segment appears as a single\n"
);
  printf(
"    edge in the triangulation.  If -q, -a, -u, or -s is selected, GTriangle\n"
);
  printf(
...
"    which segments may be subdivided into smaller edges.  If -D is\n");
  printf(
"    selected, GTriangle produces a conforming Delaunay triangulation, so\n");
  printf(
...
"    the triangulation.  Holes are specified by identifying a point inside\n");
  printf(
"    each hole.  After the triangulation is formed, GTriangle creates holes\n");
  printf(
"    by eating triangles_, spreading out from each hole point until its\n");
  printf(
...
"    enclose each hole in segments, or your whole triangulation might be\n");
  printf(
"    eaten away.  If the two triangles_ abutting a segment are eaten, the\n");
  printf(
"    segment itself is also eaten.  Do not place a hole directly on a\n");
  printf("    segment; if you do, GTriangle chooses one side of the segment\n");
  printf("    arbitrarily.\n\n");
  printf(
"    The optional fourth section lists regional attributes (to be assigned\n");
  printf(
"    to all triangles_ in a region) and regional constraints on the maximum\n");
  printf(
"    triangle area.  GTriangle reads this section only if the -A switch is\n");
  printf(
...
"    enclose the convex hull of the PSLG.  If you do not use the -c switch,\n"
  ...
  printf(
"    GTriangle eats all triangles_ that are not enclosed by segments; if you\n");
  printf(
...
"    of what can go wrong.  Segment intersections are relatively safe--\n");
  printf(
"    GTriangle calculates the intersection points for you and adds them to\n");
  printf(
...
"    doesn't become a problem.  You are tempting the fates if you have three\n"
  ...
  printf(
"    segments that cross at the same location, and expect GTriangle to figure\n"
);
  printf(
"    out where the intersection point is.  Thanks to floating-point roundoff\n"
);
  printf(
"    error, GTriangle will probably decide that the three segments intersect\n"
);
  printf(
"    at three different points, and you will find a minuscule triangle in\n");
  printf(
"    your output--unless GTriangle tries to refine the tiny triangle, uses\n");
  printf(
...
"    and manually breaking up each segment into two.  Similarly, if you\n");
  printf(
"    place a vertex at the middle of a segment, and hope that GTriangle will\n"
);
  printf(
"    break up the segment at that vertex, you might get lucky.  On the other\n"
);
  printf(
"    hand, GTriangle might decide that the vertex doesn't lie precisely on\n");
  printf(
"    the segment, and you'll have a needle-sharp triangle in your output--or\n"
);
  printf("    a lot of tiny triangles_ if you're generating a quality mesh.\n");
  printf("\n");
  printf(
"    When GTriangle reads a .poly file, it also writes a .poly file, which\n");
  printf(
...
"    mesh, and don't want segments to be missing in later triangulations.\n");
  printf("\n");
  printf("  .area files:\n");
  printf("    First line:  <# of triangles_>\n");
...
"    for mesh refinement.  As with other file formats, every triangle must\n");
  printf(
"    be represented, and the triangles_ must be numbered consecutively.  A\n");
  printf(
...
"    Following lines:  <edge #> <endpoint> <endpoint> [boundary marker]\n");
  printf("\n");
  printf(
"    Endpoints are indices into the corresponding .node file.  GTriangle can\n"
);
  printf(
...
"    The `direction' is a floating-point vector that indicates the direction\n"
  ...
  printf("  .neigh files:\n");
  printf(
"    First line:  <# of triangles_> <# of neighbors per triangle (always 3)>\n"
);
  printf(
...
"    boundary).  The first neighbor of triangle i is opposite the first\n");
  ...
  printf(
"    GTriangle can produce .neigh files (use the -n switch), but cannot read\n"
);
  printf("    them.\n\n");
...
"  finite element mesh.  You can prevent boundary markers from being written\n"
  ...
  printf("  into files produced by GTriangle by using the -B switch.\n\n");
  printf(
...
"    - Otherwise, if the vertex occurs on a boundary of the triangulation,\n");
  ...
  printf("\n");
  printf(
"  If you want GTriangle to determine for you which vertices and edges are on\n"
);
  printf(
...
"  all) in your input files.  In the output files, all boundary vertices,\n");
  ...
  printf(
"  Because GTriangle can read and refine its own triangulations, input\n");
  printf(
"  and output files have iteration numbers.  For instance, GTriangle might\n");
  printf(
...
"  triangulation, and output the files mesh.4.node, mesh.4.ele, and\n");
  ...
  printf(
"  their iteration number is zero; hence, GTriangle might read the file\n");
  printf(
...
"  points, a .node file is written.  This can be quite convenient for\n");
  ...
  printf("Examples of How to Use GTriangle:\n\n");
  printf(
...
"  object.node), generates a mesh whose angles are all between 31.5 and 117\n"
  ...
  printf(
"  degrees and whose triangles_ all have areas of 0.1 or less, and writes the\n"
);
  printf(
...
"  other vertices but 4 have been marked to indicate that they lie on a\n");
  ...
  printf("\n");
  printf("  Here is the output file `box.1.ele', with twelve triangles_.\n");
  printf("\n");
  printf("    12  3  0\n");
  printf("       1     5   6   9\n");
  printf("       2    10   3   7\n");
...
"  newly added vertices.  Note also that <# of vertices> is set to zero to\n");
  ...
  printf("    # Generated by triangle -pqc box.poly\n");
  printf("\n");
  printf("Refinement and ClipperArea Constraints:\n");
  printf("\n");
  printf(
...
"  (without a number following).  Each triangle's area constraint is applied\n"
  ...
  printf(
"  to that triangle.  ClipperArea constraints tend to diffuse as the mesh is\n");
  printf(
"  refined, so if there are large variations in area constraint between\n");
  printf(
"  adjacent triangles_, you may not get the results you want.  In that case,\n"
);
  printf(
"  consider instead using the -u switch and writing a C procedure that\n");
  printf("  determines which triangles_ are too large.\n\n");
  printf(
...
"  (segment-bounded) region in which a point falls, whereas a maximum area\n");
  printf(
"  in an .area file applies to only one triangle.  ClipperArea constraints in .poly\n"
);
  printf(
...
"  `triangle -rpaa6.2 z.3' reads z.3.node, z.3.ele, z.3.poly, and z.3.area.\n"
  ...
  printf(
"  After reconstructing the mesh and its subsegments, GTriangle refines the\n");
  printf(
"  mesh so that no triangle has area greater than 6.2, and furthermore the\n");
  printf(
"  triangles_ satisfy the maximum area constraints in z.3.area.  No angle\n");
  printf(
...
"  x.2' creates a sequence of successively finer meshes x.1, x.2, and x.3,\n");
  ...
  printf(
"  If the input is a vertex set (not a PSLG), GTriangle produces its convex\n");
  printf(
...
"  If the input is an unconstrained mesh (you are using the -r switch but\n");
  printf(
"  not the -p switch), GTriangle produces a list of its boundary edges\n");
  printf(
...
"  rays.  (The choice of filenames makes it easy to run the set of Voronoi\n");
  printf("  vertices through GTriangle, if so desired.)\n\n");
  printf(
...
"  near-degenerate, the Voronoi diagram may have duplicate vertices or\n");
  ...
  printf(
"  The result is a valid Voronoi diagram only if GTriangle's output is a true\n"
);
  printf(
...
"  CCDT, or if it has holes or concavities.  If the triangulated domain is\n");
  printf(
"  convex and has no holes, you can use -D switch to force GTriangle to\n");
  printf(
"  construct a conforming Delaunay triangulation instead of a CCDT, so the\n");
  printf("  Voronoi diagram will be valid.\n\n");
  printf("Mesh Topology:\n\n");
  printf(
"  You may wish to know which triangles_ are adjacent to a certain Delaunay\n");
  printf(
...
"  the corresponding .v.edge file, and is rotated 90 degrees counterclock-\n");
  printf(
"  wise from the Voronoi edge.  GTriangle j of an .ele file is the dual of\n");
  printf(
"  vertex j of the corresponding .v.node file.  Voronoi cell k is the dual\n");
  printf("  of vertex k of the corresponding .node file.\n\n");
  printf(
"  Hence, to find the triangles_ adjacent to a Delaunay edge, look at the\n");
  printf(
"  vertices of the corresponding Voronoi edge.  If the endpoints of a\n");
  printf(
"  Voronoi edge are Voronoi vertices 2 and 6 respectively, then triangles_ 2\n"
);
  printf(
...
"  respectively.  To find which Voronoi cells are adjacent to each other,\n");
  ...
  printf(
"  GTriangle does not write a list of the edges adjoining each Voronoi cell,\n"
);
  printf(
...
"  Voronoi vertices attached to it.  You might find this more convenient\n");
  ...
  printf(
"  GTriangle generates meshes with subparametric quadratic elements if the\n");
  printf(
"  -o2 switch is specified.  Quadratic elements have six nodes per element,\n"
);
  printf(
"  rather than three.  `Subparametric' means that the edges of the triangles_\n"
);
  printf(
...
"  If two input segments adjoin each other at a small angle, clearly the -q\n"
  ...
  printf(
"  switch cannot remove the small angle.  Moreover, GTriangle may have no\n");
  printf(
"  choice but to generate additional triangles_ whose smallest angles are\n");
  printf(
"  smaller than the specified bound.  However, these triangles_ only appear\n");
  printf(
"  between input segments separated by small angles.  Moreover, if you\n");
  printf(
"  request a minimum angle of theta degrees, GTriangle will generally produce\n"
);
  printf(
"  no angle larger than 180 - 2 theta, even if it is forced to compromise on\n"
  ...
  printf("Statistics:\n\n");
  printf(
"  After generating a mesh, GTriangle prints a count of entities in the\n");
  printf(
"  output mesh, including the number of vertices, triangles_, edges, exterior\n"
);
  printf(
...
"  input segments not on the boundary), and total subsegments.  If you've\n");
  printf(
"  forgotten the statistics for an existing mesh, run GTriangle on that mesh\n"
);
  printf(
...
"  of memory use, the number of calls to geometric predicates, and\n");
  printf(
"  histograms of the angles and the aspect ratios of the triangles_ in the\n");
  printf("  mesh.\n\n");
  printf("Exact Arithmetic:\n\n");
  printf(
"  GTriangle uses adaptive exact arithmetic to perform what computational\n");
  printf(
...
"  floating-point registers.  These must be reconfigured so their precision\n"
  ...
  printf(
"  is reduced to memory precision.  GTriangle does this if it is compiled\n");
...
"  Another pair of problems not solved by the exact arithmetic routines is\n");
  printf(
"  underflow and overflow.  If GTriangle is compiled for double precision\n");
  printf(
"  arithmetic, I believe that GTriangle's geometric predicates work correctly\n"
);
  printf(
...
"  being performed exactly, while overflow typically causes a floating\n");
  printf("  exception.\n\n");
  printf("Calling GTriangle from Another Program:\n\n");
  printf("  Read the file triangle.h for details.\n\n");
  printf("Troubleshooting:\n\n");
  printf("  Please read this section before mailing me bugs.\n\n");
  printf("  `My output mesh has no triangles_!'\n\n");
  printf(
...
"    of bounding segments, or forgotten to use the -c switch.  Or you may\n");
  printf(
"    have placed a hole badly, thereby eating all your triangles_.  To test\n");
...
"    Alternatively, all your input vertices may be collinear, in which case\n"
  ...
  printf("  `GTriangle doesn't terminate, or just crashes.'\n\n");
  printf(
"    Bad things can happen when triangles_ get so small that the distance\n");
  printf(
"    between their vertices isn't much larger than the precision of your\n");
  printf(
"    machine's arithmetic.  If you've compiled GTriangle for single-precision\n"
);
  printf(
...
"    between 6247133 and 6247134, you're not leaving much floating-point\n");
  printf("    precision for GTriangle to work with.\n\n");
  printf(
...
"    that a tiny angle is being formed, you might never discover why\n");
  printf(
"    GTriangle is crashing.  To check for this possibility, use the -S switch\n"
);
  printf(
"    (with an appropriate limit on the number of Steiner points, found by\n");
  printf(
"    trial-and-error) to stop GTriangle early, and view the output .poly file\n"
);
  printf(
...
"    Zoom in closely, as such segments might look like a single segment from\n"
  ...
  printf("    a distance.\n\n");
  printf(
"    If some of the input values are too large, GTriangle may suffer a\n");
  printf(
...
"    orientation or incircle test.  (Read the section on exact arithmetic\n");
  printf(
"    above.)  Again, I recommend compiling GTriangle for double (rather\n");
...
"    inside the convex hull.  If so, the vertex and the nearby convex hull\n");
  printf(
"    edge form an extremely thin triangle.  When GTriangle tries to refine\n");
  printf(
"    the mesh to enforce angle and area constraints, GTriangle might generate\n"
);
  printf(
"    extremely tiny triangles_, or it might fail because of insufficient\n");
...
"    enclosed by segments.  In any case, you can solve the problem by not\n");
  ...
  printf(
"  `GTriangle executes without incident, but when I look at the resulting\n");
  printf(
"  mesh, it has overlapping triangles_ or other geometric inconsistencies.'\n");
  printf("\n");
  printf(
"    If you select the -X switch, GTriangle occasionally makes mistakes due\n");
  printf(
...
"  Strange things can happen if you've taken liberties with your PSLG.  Do\n");
  printf(
"  you have a vertex lying in the middle of a segment?  GTriangle sometimes\n");
  printf(
...
"  endpoints, try not to let the intersections fall extremely close to PSLG\n"
  ...
  printf(
"  If you have problems refining a triangulation not produced by GTriangle:\n");
  printf(
"  Are you sure the triangulation is geometrically valid?  Is it formatted\n");
  printf(
"  correctly for GTriangle?  Are the triangles_ all listed so the first three\n"
);
  printf(
"  vertices are their corners in counterclockwise order?  Are all of the\n");
  printf(
"  triangles_ constrained Delaunay?  GTriangle's Delaunay refinement algorithm\n"
);
  printf("  assumes that it starts with a CDT.\n\n");
  printf("Show Me:\n\n");
  printf(
"  GTriangle comes with a separate program named `Show Me', whose primary\n");
  printf(
...
"  purpose is to check the validity of your input files, and do so more\n");
  printf(
"  thoroughly than GTriangle does.  Unlike GTriangle, Show Me requires that\n");
  printf(
"  you have the X Windows system.  Sorry, Microsoft Windows users.\n");
  printf("\n");
  printf("GTriangle on the Web:\n");
  printf("\n");
  ...
  printf("\n");
  printf(
"  If you use GTriangle, and especially if you use it to accomplish real\n");
  printf(
"  work, I would like very much to hear from you.  A short letter or email\n");
  printf(
"  (to jrs@cs.berkeley.edu) describing how you use GTriangle will mean a lot\n"
);
  printf(
...
"  you.  Also, I can put you on a list to receive email whenever a new\n");
  printf("  version of GTriangle is available.\n\n");
  printf(
"  If you use a mesh generated by GTriangle in a publication, please include\n"
);
  printf(
"  an acknowledgment as well.  And please spell GTriangle with a capital `T'!\n"
);
  printf(
"  If you want to include a citation, use `Jonathan Richard Shewchuk,\n");
  printf(
"  ``GTriangle: Engineering a 2D Quality Mesh Generator and Delaunay\n");
  printf(
...
"  Of course, I can take credit for only a fraction of the ideas that made\n");
  printf(
"  this mesh generator possible.  GTriangle owes its existence to the efforts\n"
);
  printf(
...
void internalerror()
{
  ...
  printf("    command line you used to run GTriangle.\n");
  triexit(1);
}
...
#define STARTINDEX 1
  ...
  if (b->jettison && b->nonodewritten && !b->quiet) {
    printf("Warning:  -j and -paramN switches are somewhat incompatible.\n");
...

{
  ...
  if (b->vararea) {
    printf("    ClipperArea constraint:  %.4g\n", areabound(*t));
  }
}
...
/*  the triangulation.  Also, `dummytri' points to one of the triangles_ on   */
...
/*  be bonded (at paramRadius zero) to some triangle on the convex hull of the    */
...

{
  ...
  /* Align `dummytri' on a `triangles_.alignbytes'-byte boundary. */
  alignptr = (unsigned long) m->dummytribase;
  m->dummytri = (triangle *)
    (alignptr + (unsigned long) m->triangles.alignbytes -
     (alignptr % (unsigned long) m->triangles.alignbytes));
  /* Initialize the three adjoining triangles_ to be "outer space."  These  */
  ...
  if (b->usesegments) {
    ...
    /* Initialize the two adjoining triangles_ to be "outer space." */
...

{
  ...
  /*   pointers to other triangles_, three pointers to corners, and possibly */
...

{
  ...
  /*   triangles_ when traversing the list of all triangles_.            */
...
/*****************************************************************************/
/*                                                                           */
/*  triangletraverse()   Traverse the triangles_, skipping dead ones.         */
/*                                                                           */
/*****************************************************************************/
...

{
  ...
  /* Initialize the three adjoining triangles_ to be "outer space". */
...
/*****************************************************************************/
/*                                                                           */
/*  robust_fast_expansion_sum_zeroelim()   Sum two expansions, eliminating zero */
/*                                  components from the output expansion.    */
/*                                                                           */
/*  Sets h = e + fs.  See my Robust Predicates paper for details.            */
/*                                                                           */
...
#ifdef ANSI_DECLARATORS
int robust_fast_expansion_sum_zeroelim(int elen, REAL *e, int flen, REAL *f, REAL *h)
#else /* not ANSI_DECLARATORS */
int robust_fast_expansion_sum_zeroelim(elen, e, flen, f, h)  /* h cannot be e or fs. */
int elen;
REAL *e;
int flen;
REAL *f;
...
/*****************************************************************************/
/*                                                                           */
/*  robust_scale_expansion_zeroelim()   Multiply an expansion by a scalar,      */
...
#ifdef ANSI_DECLARATORS
int robust_scale_expansion_zeroelim(int elen, REAL *e, REAL b, REAL *h)
#else /* not ANSI_DECLARATORS */
int robust_scale_expansion_zeroelim(elen, e, b, h)   /* e and h cannot be the same. */
int elen;
REAL *e;
REAL b;
REAL *h;
...
/*****************************************************************************/
/*                                                                           */
/*  robust_estimate()   Produce a one-word estimate of an expansion's value.    */
/*                                                                           */
...
#ifdef ANSI_DECLARATORS
REAL robust_estimate(int elen, REAL *e)
#else /* not ANSI_DECLARATORS */
REAL robust_estimate(elen, e)
int elen;
REAL *e;
#endif /* not ANSI_DECLARATORS */
...

{
  ...
  u[3] = u3;
  C1length = robust_fast_expansion_sum_zeroelim(4, B, 4, u, C1);

  Two_Product(acx, bcytail, s1, s0);
  Two_Product(acy, bcxtail, t1, t0);
  Two_Two_Diff(s1, s0, t1, t0, u3, u[2], u[1], u[0]);
  u[3] = u3;
  C2length = robust_fast_expansion_sum_zeroelim(C1length, C1, 4, u, C2);

  Two_Product(acxtail, bcytail, s1, s0);
  Two_Product(acytail, bcxtail, t1, t0);
  Two_Two_Diff(s1, s0, t1, t0, u3, u[2], u[1], u[0]);
  u[3] = u3;
  Dlength = robust_fast_expansion_sum_zeroelim(C2length, C2, 4, u, D);
...
/*****************************************************************************/
/*                                                                           */
/*  robust_incircle()   Return a positive value if the point pd lies inside the */
...
#ifdef ANSI_DECLARATORS
REAL robust_incircleadapt(vertex pa, vertex pb, vertex pc, vertex pd, REAL permanent)
#else /* not ANSI_DECLARATORS */
REAL robust_incircleadapt(pa, pb, pc, pd, permanent)
vertex pa;
vertex pb;
vertex pc;
vertex pd;
...

{
  ...
  bc[3] = bc3;
  axbclen = robust_scale_expansion_zeroelim(4, bc, adx, axbc);
  axxbclen = robust_scale_expansion_zeroelim(axbclen, axbc, adx, axxbc);
  aybclen = robust_scale_expansion_zeroelim(4, bc, ady, aybc);
  ayybclen = robust_scale_expansion_zeroelim(aybclen, aybc, ady, ayybc);
  alen = robust_fast_expansion_sum_zeroelim(axxbclen, axxbc, ayybclen, ayybc, adet);

  Two_Product(cdx, ady, cdxady1, cdxady0);
  Two_Product(adx, cdy, adxcdy1, adxcdy0);
  Two_Two_Diff(cdxady1, cdxady0, adxcdy1, adxcdy0, ca3, ca[2], ca[1], ca[0]);
  ca[3] = ca3;
  bxcalen = robust_scale_expansion_zeroelim(4, ca, bdx, bxca);
  bxxcalen = robust_scale_expansion_zeroelim(bxcalen, bxca, bdx, bxxca);
  bycalen = robust_scale_expansion_zeroelim(4, ca, bdy, byca);
  byycalen = robust_scale_expansion_zeroelim(bycalen, byca, bdy, byyca);
  blen = robust_fast_expansion_sum_zeroelim(bxxcalen, bxxca, byycalen, byyca, bdet);

  Two_Product(adx, bdy, adxbdy1, adxbdy0);
  Two_Product(bdx, ady, bdxady1, bdxady0);
  Two_Two_Diff(adxbdy1, adxbdy0, bdxady1, bdxady0, ab3, ab[2], ab[1], ab[0]);
  ab[3] = ab3;
  cxablen = robust_scale_expansion_zeroelim(4, ab, cdx, cxab);
  cxxablen = robust_scale_expansion_zeroelim(cxablen, cxab, cdx, cxxab);
  cyablen = robust_scale_expansion_zeroelim(4, ab, cdy, cyab);
  cyyablen = robust_scale_expansion_zeroelim(cyablen, cyab, cdy, cyyab);
  clen = robust_fast_expansion_sum_zeroelim(cxxablen, cxxab, cyyablen, cyyab, cdet);

  ablen = robust_fast_expansion_sum_zeroelim(alen, adet, blen, bdet, abdet);
  finlength = robust_fast_expansion_sum_zeroelim(ablen, abdet, clen, cdet, fin1);

  det = robust_estimate(finlength, fin1);
  ...
  if (adxtail != 0.0) {
    axtbclen = robust_scale_expansion_zeroelim(4, bc, adxtail, axtbc);
    temp16alen = robust_scale_expansion_zeroelim(axtbclen, axtbc, 2.0 * adx,
                                          temp16a);

    axtcclen = robust_scale_expansion_zeroelim(4, cc, adxtail, axtcc);
    temp16blen = robust_scale_expansion_zeroelim(axtcclen, axtcc, bdy, temp16b);

    axtbblen = robust_scale_expansion_zeroelim(4, bb, adxtail, axtbb);
    temp16clen = robust_scale_expansion_zeroelim(axtbblen, axtbb, -cdy, temp16c);

    temp32alen = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                            temp16blen, temp16b, temp32a);
    temp48len = robust_fast_expansion_sum_zeroelim(temp16clen, temp16c,
                                            temp32alen, temp32a, temp48);
    finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
                                            temp48, finother);
    finswap = finnow; finnow = finother; finother = finswap;
  }
  if (adytail != 0.0) {
    aytbclen = robust_scale_expansion_zeroelim(4, bc, adytail, aytbc);
    temp16alen = robust_scale_expansion_zeroelim(aytbclen, aytbc, 2.0 * ady,
                                          temp16a);

    aytbblen = robust_scale_expansion_zeroelim(4, bb, adytail, aytbb);
    temp16blen = robust_scale_expansion_zeroelim(aytbblen, aytbb, cdx, temp16b);

    aytcclen = robust_scale_expansion_zeroelim(4, cc, adytail, aytcc);
    temp16clen = robust_scale_expansion_zeroelim(aytcclen, aytcc, -bdx, temp16c);
  ...
  if (bdxtail != 0.0) {
    bxtcalen = robust_scale_expansion_zeroelim(4, ca, bdxtail, bxtca);
    temp16alen = robust_scale_expansion_zeroelim(bxtcalen, bxtca, 2.0 * bdx,
                                          temp16a);

    bxtaalen = robust_scale_expansion_zeroelim(4, aa, bdxtail, bxtaa);
    temp16blen = robust_scale_expansion_zeroelim(bxtaalen, bxtaa, cdy, temp16b);

    bxtcclen = robust_scale_expansion_zeroelim(4, cc, bdxtail, bxtcc);
    temp16clen = robust_scale_expansion_zeroelim(bxtcclen, bxtcc, -ady, temp16c);
  ...
  if (bdytail != 0.0) {
    bytcalen = robust_scale_expansion_zeroelim(4, ca, bdytail, bytca);
    temp16alen = robust_scale_expansion_zeroelim(bytcalen, bytca, 2.0 * bdy,
                                          temp16a);

    bytcclen = robust_scale_expansion_zeroelim(4, cc, bdytail, bytcc);
    temp16blen = robust_scale_expansion_zeroelim(bytcclen, bytcc, adx, temp16b);

    bytaalen = robust_scale_expansion_zeroelim(4, aa, bdytail, bytaa);
    temp16clen = robust_scale_expansion_zeroelim(bytaalen, bytaa, -cdx, temp16c);
  ...
  if (cdxtail != 0.0) {
    cxtablen = robust_scale_expansion_zeroelim(4, ab, cdxtail, cxtab);
    temp16alen = robust_scale_expansion_zeroelim(cxtablen, cxtab, 2.0 * cdx,
                                          temp16a);

    cxtbblen = robust_scale_expansion_zeroelim(4, bb, cdxtail, cxtbb);
    temp16blen = robust_scale_expansion_zeroelim(cxtbblen, cxtbb, ady, temp16b);

    cxtaalen = robust_scale_expansion_zeroelim(4, aa, cdxtail, cxtaa);
    temp16clen = robust_scale_expansion_zeroelim(cxtaalen, cxtaa, -bdy, temp16c);
  ...
  if (cdytail != 0.0) {
    cytablen = robust_scale_expansion_zeroelim(4, ab, cdytail, cytab);
    temp16alen = robust_scale_expansion_zeroelim(cytablen, cytab, 2.0 * cdy,
                                          temp16a);

    cytaalen = robust_scale_expansion_zeroelim(4, aa, cdytail, cytaa);
    temp16blen = robust_scale_expansion_zeroelim(cytaalen, cytaa, bdx, temp16b);

    cytbblen = robust_scale_expansion_zeroelim(4, bb, cdytail, cytbb);
    temp16clen = robust_scale_expansion_zeroelim(cytbblen, cytbb, -adx, temp16c);
  ...
  if ((adxtail != 0.0) || (adytail != 0.0)) {
    if ((bdxtail != 0.0) || (bdytail != 0.0)
      ...
      v[3] = v3;
      bctlen = robust_fast_expansion_sum_zeroelim(4, u, 4, v, bct);
    ...
    if (adxtail != 0.0) {
      temp16alen = robust_scale_expansion_zeroelim(axtbclen, axtbc, adxtail, temp16a);
      axtbctlen = robust_scale_expansion_zeroelim(bctlen, bct, adxtail, axtbct);
      temp32alen = robust_scale_expansion_zeroelim(axtbctlen, axtbct, 2.0 * adx,
                                            temp32a);
      temp48len = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
      finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
                                              temp48, finother);
      finswap = finnow; finnow = finother; finother = finswap;
      if (bdytail != 0.0) {
        temp8len = robust_scale_expansion_zeroelim(4, cc, adxtail, temp8);
        temp16alen = robust_scale_expansion_zeroelim(temp8len, temp8, bdytail,
                                              temp16a);
        finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                temp16a, finother);
        finswap = finnow; finnow = finother; finother = finswap;
      }
      if (cdytail != 0.0) {
        temp8len = robust_scale_expansion_zeroelim(4, bb, -adxtail, temp8);
        temp16alen = robust_scale_expansion_zeroelim(temp8len, temp8, cdytail,
                                              temp16a);
        finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                temp16a, finother);
        finswap = finnow; finnow = finother; finother = finswap;
      ...
      temp32alen = robust_scale_expansion_zeroelim(axtbctlen, axtbct, adxtail,
                                            temp32a);
      axtbcttlen = robust_scale_expansion_zeroelim(bcttlen, bctt, adxtail, axtbctt);
      temp16alen = robust_scale_expansion_zeroelim(axtbcttlen, axtbctt, 2.0 * adx,
                                            temp16a);
      temp16blen = robust_scale_expansion_zeroelim(axtbcttlen, axtbctt, adxtail,
                                            temp16b);
      temp32blen = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
      temp64len = robust_fast_expansion_sum_zeroelim(temp32alen, temp32a,
                                              temp32blen, temp32b, temp64);
      finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp64len,
                                              temp64, finother);
      finswap = finnow; finnow = finother; finother = finswap;
    }
    if (adytail != 0.0) {
      temp16alen = robust_scale_expansion_zeroelim(aytbclen, aytbc, adytail, temp16a);
      aytbctlen = robust_scale_expansion_zeroelim(bctlen, bct, adytail, aytbct);
      temp32alen = robust_scale_expansion_zeroelim(aytbctlen, aytbct, 2.0 * ady,
                                            temp32a);
      temp48len = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
      finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
      ...
      temp32alen = robust_scale_expansion_zeroelim(aytbctlen, aytbct, adytail,
                                            temp32a);
      aytbcttlen = robust_scale_expansion_zeroelim(bcttlen, bctt, adytail, aytbctt);
      temp16alen = robust_scale_expansion_zeroelim(aytbcttlen, aytbctt, 2.0 * ady,
                                            temp16a);
      temp16blen = robust_scale_expansion_zeroelim(aytbcttlen, aytbctt, adytail,
                                            temp16b);
      temp32blen = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
      temp64len = robust_fast_expansion_sum_zeroelim(temp32alen, temp32a,
  ...
  if ((bdxtail != 0.0) || (bdytail != 0.0)) {
    if ((cdxtail != 0.0) || (cdytail != 0.0)
      ...
      Two_Two_Sum(ti1, ti0, tj1, tj0, v3, v[2], v[1], v[0]);
      v[3] = v3;
      catlen = robust_fast_expansion_sum_zeroelim(4, u, 4, v, cat);
    ...
    if (bdxtail != 0.0) {
      temp16alen = robust_scale_expansion_zeroelim(bxtcalen, bxtca, bdxtail, temp16a);
      bxtcatlen = robust_scale_expansion_zeroelim(catlen, cat, bdxtail, bxtcat);
      temp32alen = robust_scale_expansion_zeroelim(bxtcatlen, bxtcat, 2.0 * bdx,
                                            temp32a);
      temp48len = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
      finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
      ...
      finswap = finnow; finnow = finother; finother = finswap;
      if (cdytail != 0.0) {
        temp8len = robust_scale_expansion_zeroelim(4, aa, bdxtail, temp8);
        temp16alen = robust_scale_expansion_zeroelim(temp8len, temp8, cdytail,
                                              temp16a);
        finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                temp16a, finother);
      ...
      if (adytail != 0.0) {
        temp8len = robust_scale_expansion_zeroelim(4, cc, -bdxtail, temp8);
        temp16alen = robust_scale_expansion_zeroelim(temp8len, temp8, adytail,
                                              temp16a);
        finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                temp16a, finother);
        finswap = finnow; finnow = finother; finother = finswap;
      ...
      temp32alen = robust_scale_expansion_zeroelim(bxtcatlen, bxtcat, bdxtail,
                                            temp32a);
      bxtcattlen = robust_scale_expansion_zeroelim(cattlen, catt, bdxtail, bxtcatt);
      temp16alen = robust_scale_expansion_zeroelim(bxtcattlen, bxtcatt, 2.0 * bdx,
                                            temp16a);
      temp16blen = robust_scale_expansion_zeroelim(bxtcattlen, bxtcatt, bdxtail,
                                            temp16b);
      temp32blen = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
      temp64len = robust_fast_expansion_sum_zeroelim(temp32alen, temp32a,
    ...
    if (bdytail != 0.0) {
      temp16alen = robust_scale_expansion_zeroelim(bytcalen, bytca, bdytail, temp16a);
      bytcatlen = robust_scale_expansion_zeroelim(catlen, cat, bdytail, bytcat);
      temp32alen = robust_scale_expansion_zeroelim(bytcatlen, bytcat, 2.0 * bdy,
                                            temp32a);
      temp48len = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
      finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
      ...
      temp32alen = robust_scale_expansion_zeroelim(bytcatlen, bytcat, bdytail,
                                            temp32a);
      bytcattlen = robust_scale_expansion_zeroelim(cattlen, catt, bdytail, bytcatt);
      temp16alen = robust_scale_expansion_zeroelim(bytcattlen, bytcatt, 2.0 * bdy,
                                            temp16a);
      temp16blen = robust_scale_expansion_zeroelim(bytcattlen, bytcatt, bdytail,
                                            temp16b);
      temp32blen = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
      temp64len = robust_fast_expansion_sum_zeroelim(temp32alen, temp32a,
  ...
  if ((cdxtail != 0.0) || (cdytail != 0.0)) {
    if ((adxtail != 0.0) || (adytail != 0.0)
      ...
      Two_Two_Sum(ti1, ti0, tj1, tj0, v3, v[2], v[1], v[0]);
      v[3] = v3;
      abtlen = robust_fast_expansion_sum_zeroelim(4, u, 4, v, abt);
    ...
    if (cdxtail != 0.0) {
      temp16alen = robust_scale_expansion_zeroelim(cxtablen, cxtab, cdxtail, temp16a);
      cxtabtlen = robust_scale_expansion_zeroelim(abtlen, abt, cdxtail, cxtabt);
      temp32alen = robust_scale_expansion_zeroelim(cxtabtlen, cxtabt, 2.0 * cdx,
                                            temp32a);
      temp48len = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
      finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
      ...
      finswap = finnow; finnow = finother; finother = finswap;
      if (adytail != 0.0) {
        temp8len = robust_scale_expansion_zeroelim(4, bb, cdxtail, temp8);
        temp16alen = robust_scale_expansion_zeroelim(temp8len, temp8, adytail,
                                              temp16a);
        finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                temp16a, finother);
      ...
      if (bdytail != 0.0) {
        temp8len = robust_scale_expansion_zeroelim(4, aa, -cdxtail, temp8);
        temp16alen = robust_scale_expansion_zeroelim(temp8len, temp8, bdytail,
                                              temp16a);
        finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                temp16a, finother);
      ...
      temp32alen = robust_scale_expansion_zeroelim(cxtabtlen, cxtabt, cdxtail,
                                            temp32a);
      cxtabttlen = robust_scale_expansion_zeroelim(abttlen, abtt, cdxtail, cxtabtt);
      temp16alen = robust_scale_expansion_zeroelim(cxtabttlen, cxtabtt, 2.0 * cdx,
                                            temp16a);
      temp16blen = robust_scale_expansion_zeroelim(cxtabttlen, cxtabtt, cdxtail,
                                            temp16b);
      temp32blen = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
      temp64len = robust_fast_expansion_sum_zeroelim(temp32alen, temp32a,
    ...
    if (cdytail != 0.0) {
      temp16alen = robust_scale_expansion_zeroelim(cytablen, cytab, cdytail, temp16a);
      cytabtlen = robust_scale_expansion_zeroelim(abtlen, abt, cdytail, cytabt);
      temp32alen = robust_scale_expansion_zeroelim(cytabtlen, cytabt, 2.0 * cdy,
                                            temp32a);
      temp48len = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
      finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
      ...
      temp32alen = robust_scale_expansion_zeroelim(cytabtlen, cytabt, cdytail,
                                            temp32a);
      cytabttlen = robust_scale_expansion_zeroelim(abttlen, abtt, cdytail, cytabtt);
      temp16alen = robust_scale_expansion_zeroelim(cytabttlen, cytabtt, 2.0 * cdy,
                                            temp16a);
      temp16blen = robust_scale_expansion_zeroelim(cytabttlen, cytabtt, cdytail,
                                            temp16b);
      temp32blen = robust_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
      temp64len = robust_fast_expansion_sum_zeroelim(temp32alen, temp32a,
...
#ifdef ANSI_DECLARATORS
REAL robust_incircle(struct mesh *m, struct behavior *b,
              vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
REAL robust_incircle(m, b, pa, pb, pc, pd)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...

{
  ...
  return robust_incircleadapt(pa, pb, pc, pd, permanent);
}

/*****************************************************************************/
/*                                                                           */
/*  robust_orient3d()   Return a positive value if the point pd lies below the  */
...
#ifdef ANSI_DECLARATORS
REAL robust_orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
                   REAL aheight, REAL bheight, REAL cheight, REAL dheight,
                   REAL permanent)
#else /* not ANSI_DECLARATORS */
REAL robust_orient3dadapt(pa, pb, pc, pd,
...

{
  ...
  Two_Two_Diff(bdxcdy1, bdxcdy0, cdxbdy1, cdxbdy0, bc3, bc[2], bc[1], bc[0]);
  bc[3] = bc3;
  alen = robust_scale_expansion_zeroelim(4, bc, adheight, adet);

  Two_Product(cdx, ady, cdxady1, cdxady0);
  Two_Product(adx, cdy, adxcdy1, adxcdy0);
  Two_Two_Diff(cdxady1, cdxady0, adxcdy1, adxcdy0, ca3, ca[2], ca[1], ca[0]);
  ca[3] = ca3;
  blen = robust_scale_expansion_zeroelim(4, ca, bdheight, bdet);

  Two_Product(adx, bdy, adxbdy1, adxbdy0);
  Two_Product(bdx, ady, bdxady1, bdxady0);
  Two_Two_Diff(adxbdy1, adxbdy0, bdxady1, bdxady0, ab3, ab[2], ab[1], ab[0]);
  ab[3] = ab3;
  clen = robust_scale_expansion_zeroelim(4, ab, cdheight, cdet);
  ...
  bctlen = robust_fast_expansion_sum_zeroelim(bt_clen, bt_c, ct_blen, ct_b, bct);
  wlength = robust_scale_expansion_zeroelim(bctlen, bct, adheight, w);
  finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                          finother);
  finswap = finnow; finnow = finother; finother = finswap;

  catlen = robust_fast_expansion_sum_zeroelim(ct_alen, ct_a, at_clen, at_c, cat);
  wlength = robust_scale_expansion_zeroelim(catlen, cat, bdheight, w);
  finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                          finother);
  finswap = finnow; finnow = finother; finother = finswap;

  abtlen = robust_fast_expansion_sum_zeroelim(at_blen, at_b, bt_alen, bt_a, abt);
  wlength = robust_scale_expansion_zeroelim(abtlen, abt, cdheight, w);
  finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                          finother);
  finswap = finnow; finnow = finother; finother = finswap;

  if (adheighttail != 0.0) {
    vlength = robust_scale_expansion_zeroelim(4, bc, adheighttail, v);
    finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, vlength, v,
                                            finother);
    finswap = finnow; finnow = finother; finother = finswap;
  }
  if (bdheighttail != 0.0) {
    vlength = robust_scale_expansion_zeroelim(4, ca, bdheighttail, v);
    finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, vlength, v,
                                            finother);
    finswap = finnow; finnow = finother; finother = finswap;
  }
  if (cdheighttail != 0.0) {
    vlength = robust_scale_expansion_zeroelim(4, ab, cdheighttail, v);
    finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, vlength, v,
                                            finother);
    finswap = finnow; finnow = finother; finother = finswap;
  }
  ...
  if (adxtail != 0.0) {
    if (bdytail != 0.0) {
      ...
      u[3] = u3;
      finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                                              finother);
      finswap = finnow; finnow = finother; finother = finswap;
      if (cdheighttail != 0.0) {
  ...
  if (adheighttail != 0.0) {
    wlength = robust_scale_expansion_zeroelim(bctlen, bct, adheighttail, w);
    finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                            finother);
    finswap = finnow; finnow = finother; finother = finswap;
  }
  if (bdheighttail != 0.0) {
    wlength = robust_scale_expansion_zeroelim(catlen, cat, bdheighttail, w);
    finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                            finother);
    finswap = finnow; finnow = finother; finother = finswap;
  }
  if (cdheighttail != 0.0) {
    wlength = robust_scale_expansion_zeroelim(abtlen, abt, cdheighttail, w);
    finlength = robust_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                            finother);
    finswap = finnow; finnow = finother; finother = finswap;
  }
...
#ifdef ANSI_DECLARATORS
REAL robust_orient3d(struct mesh *m, struct behavior *b,
              vertex pa, vertex pb, vertex pc, vertex pd,
              REAL aheight, REAL bheight, REAL cheight, REAL dheight)
#else /* not ANSI_DECLARATORS */
REAL robust_orient3d(m, b, pa, pb, pc, pd, aheight, bheight, cheight, dheight)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...

{
  ...
  return robust_orient3dadapt(pa, pb, pc, pd, aheight, bheight, cheight, dheight,
                       permanent);
}
...
{
  if (b->weighted == 0) {
    return robust_incircle(m, b, pa, pb, pc, pd);
  } else if (b->weighted == 1) {
    return robust_orient3d(m, b, pa, pb, pc, pd,
                    pa[0] * pa[0] + pa[1] * pa[1] - pa[2],
                    pb[0] * pb[0] + pb[1] * pb[1] - pb[2],
                    pc[0] * pc[0] + pc[1] * pc[1] - pc[2],
                    pd[0] * pd[0] + pd[1] * pd[1] - pd[2]);
  } else {
    return robust_orient3d(m, b, pa, pb, pc, pd, pa[2], pb[2], pc[2], pd[2]);
  }
}
...

{
  ...
  /*   serves as a conservative estimate of the insertion paramRadius of the */
...

{
  ...
  horrors = 0;
  /* Run through the list of triangles_, checking each one. */
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (oppotri.tri != m->dummytri) {
        ...
        /* Check that both triangles_ agree on the identities */
        /*   of their shared vertices.                       */
        org(oppotri, oppoorg);
        dest(oppotri, oppodest);
        if ((triorg != oppodest) || (tridest != oppoorg)) {
          printf("  !! !! Mismatched edge coordinates between two triangles_:\n"
                 );
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (m->checksegments && shouldbedelaunay) {
        /* If a subsegment separates the triangles_, then the edge is */
      ...
      if (shouldbedelaunay) {
        if (nonregular(m, b, triorg, tridest, triapex, oppoapex) > 0.0) {
          if (!b->weighted) {
            printf("  !! !! Non-Delaunay pair of triangles_:\n");
            printf("    First non-Delaunay ");
            printtriangle(m, b, &triangleloop);
            printf("    Second non-Delaunay ");
          } else {
            printf("  !! !! Non-regular pair of triangles_:\n");
...
/*  just big enough to enclose two isosceles triangles_ whose bases are the   */
/*  subsegment.  Each of the two isosceles triangles_ has two angles equal    */
/*  to `b->minangle'.                                                        */
/*                                                                           */
...

{
  ...
  if (b->vararea || b->fixedarea || b->usertest) {
    ...
    if (b->fixedarea && (area > b->maxarea)) {
      /* Add this triangle to the list of bad triangles_. */
  ...
  if (angle > b->goodangle) {
    ...
    /*   triangles_ should not be split, even if they have bad angles.      */
...
/*****************************************************************************/
/*                                                                           */
/*  makevertexmap()   Construct a mapping from vertices to triangles_ to      */
/*                    improve the speed of point location for segment        */
/*                    insertion.                                             */
/*                                                                           */
/*  Traverses all the triangles_, and provides each corner of each triangle   */
/*  with a pointer to that triangle.  Of course, pointers will be            */
/*  overwritten by other pointers because (almost) each vertex is a corner   */
/*  of several triangles_, but in the end every vertex will point to some     */
...

{
  ...
  if (b->verbose) {
    printf("    Constructing mapping from vertices to triangles_.\n");
  }
  traversalinit(&m->triangles);
  triangleloop.tri = triangletraverse(m);
  while (triangleloop.tri != (triangle *) NULL) {
...

{
  ...
  /*   the number of triangles_ in the mesh.  The next bit of code assumes   */
  /*   that the number of triangles_ increases monotonically (or at least    */
  ...
  /*   from each block of triangles_ (except the first)--until we meet the */
  ...
  /*   from the first block of triangles_.                                    */
  ...
  while (totalsamplesleft > 0) {
    ...
    /* Choose `samplesleft' randomly sampled triangles_ in this block. */
    do {
...
struct mesh *m;
struct behavior *b;
struct otri *tri;             /* GEdge at which to insert the new subsegment. */
...

{
  ...
  if (newsubseg.ss == m->dummysub) {
    ...
    /* Bond new subsegment to the two triangles_ it is sandwiched between. */
...
/*  Terminology                                                              */
/*                                                                           */
/*  A "local transformation" replaces a small set of triangles_ with another  */
/*  set of triangles_.  This may or may not involve inserting or deleting a   */
/*  vertex.                                                                  */
/*                                                                           */
/*  The term "casing" is used to describe the set of triangles_ that are      */
/*  attached to the triangles_ being transformed, but are not transformed     */
...
/*****************************************************************************/
/*                                                                           */
/*  flip()   Transform two triangles_ to two different triangles_ by flipping  */
/*           an edge counterclockwise within a quadrilateral.                */
/*                                                                           */
/*  Imagine the original triangles_, abc and bad, oriented so that the        */
/*  shared edge ab lies in a horizontal plane, with the vertex b on the left */
/*  and the vertex a on the right.  The vertex c lies below the edge, and    */
/*  the vertex d lies above the edge.  The `flipedge' handle holds the edge  */
/*  ab of triangle abc, and is directed left, from vertex a to vertex b.     */
/*                                                                           */
/*  The triangles_ abc and bad are deleted and replaced by the triangles_ cdb  */
/*  and dca.  The triangles_ that represent abc and bad are NOT deallocated;  */
/*  they are reused for dca and cdb, respectively.  Hence, any handles that  */
/*  may have held the original triangles_ are still valid, although not       */
/*  directed as they were before.                                            */
/*                                                                           */
/*  Upon completion of this routine, the `flipedge' handle holds the edge    */
/*  dc of triangle dca, and is directed down, from vertex d to vertex c.     */
/*  (Hence, the two triangles_ have rotated counterclockwise.)                */
/*                                                                           */
/*  WARNING:  This transformation is geometrically valid only if the         */
/*  quadrilateral adbc is convex.  Furthermore, this transformation is       */
/*  valid only if there is not a subsegment between the triangles_ abc and    */
...

{
  ...
  if (b->verbose > 2) {
    printf("  GEdge flip results in left ");
...
/*****************************************************************************/
/*                                                                           */
/*  unflip()   Transform two triangles_ to two different triangles_ by         */
/*             flipping an edge clockwise within a quadrilateral.  Reverses  */
/*             the flip() operation so that the data structures representing */
/*             the triangles_ are back where they were before the flip().     */
/*                                                                           */
/*  Imagine the original triangles_, abc and bad, oriented so that the        */
/*  shared edge ab lies in a horizontal plane, with the vertex b on the left */
/*  and the vertex a on the right.  The vertex c lies below the edge, and    */
...
/*  (Hence, the two triangles_ have rotated clockwise.)                       */
/*                                                                           */
/*  WARNING:  This transformation is geometrically valid only if the         */
/*  quadrilateral adbc is convex.  Furthermore, this transformation is       */
/*  valid only if there is not a subsegment between the triangles_ abc and    */
...

{
  ...
  if (b->verbose > 2) {
    printf("  GEdge unflip results in left ");
    printtriangle(m, b, flipedge);
    printf("  and right ");
    printtriangle(m, b, &top);
  }
...
/*  thereby splitting the two adjacent triangles_ into four.  GEdge flips are  */
...
/*  quality triangles_.  If a newly inserted vertex encroaches upon           */
/*  subsegments, these subsegments are added to the list of subsegments to   */
/*  be split if `segmentflaws' is set.  If bad triangles_ are created, these  */
...

{
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
    ...
    /*   the edge lies on a boundary) or two triangles_ into four.       */
    ...
    /* Set the vertices of changed and new triangles_. */
    ...
    /* Bond the new triangle(s) to the surrounding triangles_. */
    ...
    for (i = 0; i < m->eextras; i++) {
      /* Set the element attributes of the new triangles_. */
      attrib = elemattribute(horiz, i);
      setelemattribute(newbotleft, i, attrib);
      setelemattribute(newbotright, i, attrib);
    }
    if (b->vararea) {
      /* Set the area constraint of the new triangles_. */
    ...
    /* Bond the new triangles_ to the surrounding triangles_. */
    ...
    if (doflip) {
      ...
      if (top.tri == m->dummytri) {
        ...
        if ((leftvertex == m->infvertex1) || (leftvertex == m->infvertex2) ||
          ...
          doflip = robust_incircle(m, b, leftvertex, newvertex, rightvertex,
                            farvertex) > 0.0;
        }
        if (doflip) {
          /* We made it!  Flip the edge `horiz' by rotating its containing */
          /*   quadrilateral (the two triangles_ adjacent to `horiz').      */
          /* Identify the casing of the quadrilateral. */
          lprev(top, topleft);
          sym(topleft, toplcasing);
          lnext(top, topright);
          ...
          setapex(top, leftvertex);
          for (i = 0; i < m->eextras; i++) {
            /* Take the average of the two triangles_' attributes. */
          ...
          if (b->vararea) {
            if ((areabound(top) <= 0.0) || (areabound(horiz) <= 0.0)) {
              ...
              /* Take the average of the two triangles_' area constraints.    */
          ...
          if (newvertex != (vertex) NULL) {
            ...
            /*   sometimes generates inverted triangles_ that insertvertex()  */
            /*   removes.                                                    */
/*
...
/*  triangles_ that share a common origin.  For each of these triangles_, the  */
/*  edge opposite the origin is one of the sides of the polygon.  The        */
/*  primary edge of each triangle is the edge directed from the origin to    */
/*  the destination; note that this is not the same edge that is a side of   */
/*  the polygon.  `firstedge' is the primary edge of the first triangle.     */
/*  From there, the triangles_ follow in counterclockwise order about the     */
/*  polygon, until `lastedge', the primary edge of the last triangle.        */
/*  `firstedge' and `lastedge' are probably connected to other triangles_     */
...
/*  performed, resulting in a fan of two triangles_, and an unfinished        */
...
/*  there's a lot of long segments that each cut many triangles_.  I ought to */
/*  code a faster algorithm some day.                                        */
/*                                                                           */
/*  The `edgecount' parameter is the number of sides of the polygon,         */
/*  including its base.  `triflaws' is a flag that determines whether the    */
/*  new triangles_ should be tested for quality, and enqueued if they are     */
/*  bad.                                                                     */
/*                                                                           */
/*****************************************************************************/
...

{
  ...
  for (i = 2; i <= edgecount - 2; i++) {
    ...
    if (robust_incircle(m, b, leftbasevertex, rightbasevertex, bestvertex,
                 testvertex) > 0.0) {
...
/*                                                                           */
/*  The origin of `deltri' is deleted.  The union of the triangles_ adjacent  */
/*  to this vertex is a polygon, for which the Delaunay triangulation is     */
/*  found.  Two triangles_ are removed from the mesh.                         */
/*                                                                           */
...

{
  ...
  /* Delete the two spliced-out triangles_. */
...
/*  Two triangles_ (possibly just one) are also deallocated.                  */
/*                                                                           */
/*****************************************************************************/
...

{
  ...
  while (m->lastflip != (struct flipstacker *) NULL) {
    ...
    /*   bisection of two triangles_ into four (by inserting a vertex in an */
    /*   edge), or an edge flip.                                           */
    if (m->lastflip->prevflip == (struct flipstacker *) NULL) {
      /* Restore a triangle that was split into three triangles_, */
      ...
      /* Restore two triangles_ that were split into four triangles_, */
      /*   so they are again two triangles_.                         */
...
/*  triangulation.  These bounding triangles_ all share a common apical       */
/*  vertex, which is represented by NULL and which represents nothing.       */
/*  The bounding triangles_ are linked in a circular fan about this NULL      */
/*  vertex, and the edges on the convex hull of the triangulation appear     */
/*  opposite the NULL vertex.  You might find it easiest to imagine that     */
/*  the NULL vertex is a point in 3D space behind the center of the          */
/*  triangulation, and that the bounding triangles_ form a sort of cone.      */
/*                                                                           */
/*  This bounding box makes it easy to represent degenerate cases.  For      */
/*  instance, the triangulation of two vertices is a single edge.  This edge */
/*  is represented by two bounding box triangles_, one on each "side" of the  */
/*  edge.  These triangles_ are also linked together in a fan about the NULL  */
/*  vertex.                                                                  */
/*                                                                           */
...
/*  them together.  As they are merged, some of their bounding triangles_     */
/*  are converted into real triangles_ of the triangulation.  The procedure   */
/*  pulls each hull's bounding triangles_ apart, then knits them together     */
/*  like the teeth of two gears.  The Delaunay property determines, at each  */
/*  step, whether the next "tooth" is a bounding triangle of the left hull   */
/*  or the right.  When a bounding triangle becomes real, its apex is        */
/*  changed from NULL to a real vertex.                                      */
/*                                                                           */
/*  Only two new triangles_ need to be allocated.  These become new bounding  */
/*  triangles_ at the top and bottom of the seam.  They are used to connect   */
/*  the remaining bounding triangles_ (those that have not been converted     */
/*  into real triangles_) into a single fan.                                  */
/*                                                                           */
/*  On entry, `farleft' and `innerleft' are bounding triangles_ of the left   */
/*  triangulation.  The origin of `farleft' is the leftmost vertex, and      */
/*  the destination of `innerleft' is the rightmost vertex of the            */
/*  triangulation.  Similarly, `innerright' and `farright' are bounding      */
/*  triangles_ of the right triangulation.  The origin of `innerright' and    */
...

{
  ...
  /* Fix the extreme triangles_ if necessary. */
  org(*farleft, farleftpt);
  ...
  while (1) {
    ...
    if (!leftfinished) {
      ...
      if (nextapex != (vertex) NULL) {
        ...
        badedge = robust_incircle(m, b, lowerleft, lowerright, upperleft, nextapex) >
                  0.0;
        while (badedge) {
          ...
          if (nextapex != (vertex) NULL) {
            /* Check whether the edge is Delaunay. */
            badedge = robust_incircle(m, b, lowerleft, lowerright, upperleft,
                               nextapex) > 0.0;
          } else {
    ...
    if (!rightfinished) {
      ...
      if (nextapex != (vertex) NULL) {
        /* Check whether the edge is Delaunay. */
        badedge = robust_incircle(m, b, lowerleft, lowerright, upperright, nextapex) >
                  0.0;
        while (badedge) {
          /* Eliminate the edge with an edge flip.  As a result, the     */
          ...
          if (nextapex != (vertex) NULL) {
            /* Check whether the edge is Delaunay. */
            badedge = robust_incircle(m, b, lowerleft, lowerright, upperright,
                               nextapex) > 0.0;
          } else {
            /* Avoid eating right through the triangulation. */
            badedge = 0;
    ...
    if (leftfinished || (!rightfinished &&
           (robust_incircle(m, b, upperleft, lowerleft, lowerright, upperright) >
            0.0))) {
...
/*                                                                           */
/*  On completion, `farleft' and `farright' are bounding triangles_ such that */
...

{
  ...
  if (vertices == 2) {
    ...
    /*   represented by two bounding triangles_.                  */
    ...
    /*   three bounding triangles_) or two edges (with four bounding   */
    /*   triangles_).  In either case, four triangles_ are created.     */
...

{
  ...
  if (b->verbose) {
    printf("  Removing ghost triangles_.\n");
  }
...
/*                                                                           */
/*  The triangular bounding box has three boundary triangles_ (one for each   */
/*  side of the bounding box), and a bunch of triangles_ fanning out from     */
/*  the three bounding box vertices (one triangle for each edge of the       */
/*  convex hull of the inner mesh).  This routine removes these triangles_.   */
/*                                                                           */
...

{
  ...
  if (checkedge.tri == m->dummytri) {
    ...
    /*   triangles_, and this next triangle cannot be the third one, */
  ...
  while (!otriequal(nextedge, finaledge)) {
    ...
    if (!b->poly) {
      ...
      /*   vertices are collinear, and thus all the triangles_ are part of */
...

{
  ...
  if (m->triangles.items == 0) {
    /* The input vertices were all collinear, so there are no triangles_. */
    return 0l;
  } else {
    return hulledges;
  }
...
/*  on each triangle.                                                        */
/*                                                                           */
/*  Vertices that are not corners of triangles_, such as nodes on edges of    */
/*  subparametric elements, are discarded.                                   */
/*                                                                           */
/*  This routine finds the adjacencies between triangles_ (and subsegments)   */
/*  by forming one stack of triangles_ for each vertex.  Each triangle is on  */
...

{
  ...
  m->eextras = attribs;
#else /* not TRILIBRARY */
  /* Read the triangles_ from an .ele file. */
  if (!b->quiet) {
    printf("Opening %s.\n", elefilename);
  }
  ...
  /* Read number of triangles_, number of vertices per triangle, and */
  ...
  if (b->vararea) {
    ...
    if (areaelements != m->inelements) {
      printf("Error:  %s and %s disagree on number of triangles_.\n",
  ...
  /*   triangles_ and subsegments first.                                */
  ...
  if (b->verbose) {
    printf("  Assembling triangles_.\n");
  }
  /* Read the triangles_ from the .ele file, and link */
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if ((corner[j] < b->firstnumber) ||
        ...
        printf("Error:  GTriangle %ld has an invalid vertex index.\n",
               elementnumber);
        triexit(1);
      }
    }
    ...
    for (j = 0; j < 3; j++) {
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        printf("Error:  GTriangle %ld is missing vertex %d in %s.\n",
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      /* Look for other triangles_ having this vertex. */
      ...
      if (checktri.tri != m->dummytri) {
        ...
        /* Look for other triangles_ that share an edge. */
        do {
          dest(checktri, checkdest);
          apex(checktri, checkapex);
          if (tapex == checkdest) {
            /* The two triangles_ share an edge; bond them together. */
  ...
  if (b->poly) {
    ...
    /*   to their neighboring triangles_.                    */
    boundmarker = 0;
  ...
  for (i = 0; i < m->vertices.items; i++) {
    /* Search the stack of triangles_ adjacent to a vertex. */
...
/*                    attention to stacking inverted triangles_.              */
/*                                                                           */
...
/*  triangles_.  (Read the description of constrainededge() below before      */
/*  reading on here, so you understand the algorithm.)  If the position of   */
/*  the new vertex (the origin of fixuptri) indicates that the vertex before */
/*  it on the polygon is a reflex vertex, then "stack" the triangle by       */
/*  doing nothing.  (fixuptri is an inverted triangle, which is how stacked  */
/*  triangles_ are identified.)                                               */
/*                                                                           */
...

{
  ...
  if (counterclockwise(m, b, rightvertex, leftvertex, farvertex) > 0.0) {
    ...
    /*   triangles_ to ensure it is locally Delaunay.                     */
    if (robust_incircle(m, b, leftvertex, farvertex, rightvertex, nearvertex) <=
        0.0) {
      return;
    }
  ...
  /* Recursively process the two triangles_ that result from the flip. */
...
/*                      triangulation by deleting the triangles_ it           */
...
/*  segment is deleted.  The union of these deleted triangles_ is a polygon   */
...
/*****************************************************************************/
/*                                                                           */
/*  infecthull()   Virally infect all of the triangles_ of the convex hull    */
...

{
  ...
  if (b->verbose) {
    printf("  Marking concavities (external triangles_) for elimination.\n");
  }
  /* Find a triangle handle on the hull. */
  hulltri.tri = m->dummytri;
  hulltri.orient = 0;
  ...
  /* Go once counterclockwise around the convex hull. */
  do {
    /* Ignore triangles_ that are already infected. */
    if (!infected(hulltri)) {
...
/*****************************************************************************/
/*                                                                           */
/*  plague()   Spread the virus from all infected triangles_ to any neighbors */
/*             not protected by subsegments.  Delete all infected triangles_. */
/*                                                                           */
/*  This is the procedure that actually creates holes and concavities.       */
/*                                                                           */
/*  This procedure operates in two phases.  The first phase identifies all   */
/*  the triangles_ that will die, and marks them as infected.  They are       */
/*  marked to ensure that each triangle is added to the virus pool only      */
/*  once, so the procedure will terminate.                                   */
/*                                                                           */
/*  The second phase actually eliminates the infected triangles_.  It also    */
...

{
  ...
  if (b->verbose) {
    printf("  Marking neighbors of marked triangles_.\n");
  }
  /* Loop through all the infected triangles_, spreading the virus to */
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      ...
      if ((neighbor.tri == m->dummytri) || infected(neighbor)) {
        if (neighborsubseg.ss != m->dummysub) {
          ...
          /*   neighbor, but both triangles_ are dying, so the subsegment */
          /*   dies too.                                                 */
  ...
  if (b->verbose) {
    printf("  Deleting marked triangles_.\n");
  }
  ...
  while (virusloop != (triangle **) NULL) {
    ...
    /*   dead triangles_ from their neighbors.                         */
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      sym(testtri, neighbor);
      if (neighbor.tri == m->dummytri) {
    ...
    /* Return the dead triangle to the pool of triangles_. */
...
/*  The triangles_ are marked to ensure that each triangle is added to the    */
/*  virus pool only once, so the procedure will terminate.                   */
/*                                                                           */
/*  The second phase uninfects all infected triangles_, returning them to     */
/*  normal.                                                                  */
/*                                                                           */
/*****************************************************************************/
...

{
  ...
  /* Loop through all the infected triangles_, spreading the attribute      */
  ...
  /* Uninfect all triangles_. */
  if (b->verbose > 1) {
    printf("  Unmarking marked triangles_.\n");
  }
  traversalinit(&m->viri);
  virusloop = (triangle **) traverse(&m->viri);
  while (virusloop != (triangle **) NULL) {
...
/*                 Spread the infection and kill triangles_.  Spread the      */
/*                 area constraints.                                         */
/*                                                                           */
...

{
  ...
  if (!(b->quiet || (b->noholes && b->convex))) {
    printf("Removing unwanted triangles_.\n");
  ...
  if (regions > 0) {
    /* Allocate storage for the triangles_ in which region points fall. */
  ...
  if (!b->convex) {
    /* Mark as infected any unprotected triangles_ on the boundary. */
...
void precisionerror()
{
  ...
  printf("  allowable angle so that tiny triangles_ are not created.\n");
#ifdef SINGLE
...
/*  bad triangles_ that result from inserting vertices to repair encroached   */
/*  subsegments.                                                             */
/*                                                                           */
/*****************************************************************************/
...

{
  ...
  if (b->verbose) {
    printf("  Making a list of bad triangles_.\n");
  }
  traversalinit(&m->triangles);
  triangleloop.orient = 0;
  triangleloop.tri = triangletraverse(m);
...

{
  ...
  if (!deadtri(badotri.tri) && (borg == badtri->triangorg) &&
    ...
    if (errorflag) {
      ...
      printf("This probably means that I am trying to refine triangles_\n");
...
/*                     triangles_ from the triangulation.                     */
/*                                                                           */
/*****************************************************************************/
...

{
  ...
  /* Fix encroached subsegments without noting bad triangles_. */
  splitencsegs(m, b, 0);
  ...
  if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
    /* Initialize the pool of bad triangles_. */
    poolinit(&m->badtriangles, sizeof(struct badtriang), BADTRIPERBLOCK,
             BADTRIPERBLOCK, 0);
    /* Initialize the queues of bad triangles_. */
    for (i = 0; i < 4096; i++) {
      m->queuefront[i] = (struct badtriang *) NULL;
    }
    m->firstnonemptyq = -1;
    /* Test all triangles_ to see if they're bad. */
    tallyfaces(m, b);
    /* Initialize the pool of recently flipped triangles_. */
    poolinit(&m->flipstackers, sizeof(struct flipstacker), FLIPSTACKERPERBLOCK,
             FLIPSTACKERPERBLOCK, 0);
    m->checkquality = 1;
    if (b->verbose) {
      printf("  Splitting bad triangles_.\n");
    }
    while ((m->badtriangles.items > 0) && (m->steinerleft != 0)) {
      ...
      if (m->badsubsegs.items > 0) {
        ...
        /*   Record any new bad triangles_ that result.   */
        splitencsegs(m, b, 1);
      } else {
  ...
  /*   and have no low-quality triangles_.                                   */
...

{
  ...
  if (!b->quiet) {
    printf("Adding vertices for second-order triangles_.\n");
  }
  ...
  traversalinit(&m->triangles);
  triangleloop.tri = triangletraverse(m);
  /* To loop over the set of edges, loop over all triangles_, and look at   */
...

{
  ...
  if (m->mesh_dim != 2) {
    printf("Error:  GTriangle only works with two-dimensional meshes.\n");
    triexit(1);
  }
  if (m->nextras == 0) {
    b->weighted = 0;
...
/*****************************************************************************/
/*                                                                           */
/*  writeelements()   Write the triangles_ to an .ele file.                   */
/*                                                                           */
/*****************************************************************************/
...

{
...
#ifdef TRILIBRARY
  if (!b->quiet) {
    printf("Writing triangles_.\n");
  }
  /* Allocate memory for output triangles_ if necessary. */
  ...
  /* Number of triangles_, vertices per triangle, attributes per triangle. */
...
#else /* not TRILIBRARY */
      /* GTriangle number, indices for three vertices. */
...
#else /* not TRILIBRARY */
      /* GTriangle number, indices for six vertices. */
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
        ...
        if (b->nobound) {
#ifndef TRILIBRARY
          /* GEdge number, indices of two endpoints. */
          fprintf(outfile, "%4ld   %d  %d\n", edgenumber,
                  vertexmark(p1), vertexmark(p2));
#endif /* not TRILIBRARY */
        } else {
          /* GEdge number, indices of two endpoints, and a boundary marker. */
...
/*  triangles_, and the Voronoi edges are listed by traversing the Delaunay   */
/*  edges.                                                                   */
/*                                                                           */
...

{
  ...
  *vpointmarkerlist = (int *) NULL;
  ...
  /* Number of triangles_, two dimensions, number of vertex attributes, */
  /*   no markers.                                                     */
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
        ...
        if (trisym.tri == m->dummytri) {
          ...
          dest(triangleloop, tdest);
#ifdef TRILIBRARY
          /* Copy an infinite ray.  GIndex of one endpoint, and -1. */
          elist[coordindex] = p1;
          normlist[coordindex++] = tdest[1] - torg[1];
          elist[coordindex] = -1;
          normlist[coordindex++] = torg[0] - tdest[0];
#else /* not TRILIBRARY */
          /* Write an infinite ray.  GEdge number, index of one endpoint, -1, */
...

{
  ...
  /* Number of triangles_, three neighbors per triangle. */
  fprintf(outfile, "%ld  %d\n", m->triangles.items, 3);
#endif /* not TRILIBRARY */
...
#else /* not TRILIBRARY */
    /* GTriangle number, neighboring triangle numbers. */
    fprintf(outfile, "%4ld    %d  %d  %d\n", elementnumber,
...

{
  ...
  /* Number of vertices, triangles_, and edges. */
...

{
  ...
  printf("  GTriangle aspect ratio histogram:\n");
...

{
  ...
  if (b->verbose) {
    ...
    printf("  Maximum number of triangles_: %ld\n", m->triangles.maxitems);
    ...
    if (m->badtriangles.maxitems > 0) {
      printf("  Maximum number of bad triangles_: %ld\n",
...
/*  - Promote the mesh's linear triangles_ to higher order elements (-o).     */
...
#endif /* not TRILIBRARY */
#ifndef NO_TIMER
  /* Variables for timing the performance of GTriangle.  The types are */
