...
void parsers::MapParser::convertMapToFade2DMesh(const std::string &filename, Fade2DMesh &fade2DMesh, int &numObst) {
...
void parsers::MapParser::convertMapToMergedMesh(const std::string &filename, parsers::MergedMesh &mergedMesh) {
    ...
    int obstacles = 0;
    convertMapToFade2DMesh(filename, fade2DMesh, obstacles);
...
parsers::MapParser::convertFade2DMeshToGeomMesh(const parsers::Fade2DMesh &fade2DMesh, parsers::GeomMesh &geomMesh) {
    ...
    // Convert fade2d mesh's obstacles to draw mesh's obstacles
...
parsers::MapParser::readGeomMeshFromIronHarvestMesh(const std::string &filename, parsers::GeomMesh &geomMesh){
    ...
    for(int i = 0; i < vertexCount; i++){
        ifs >> currentVertex.point.x;
        ifs >> currentVertex.point.y;
    ...
    for(int i = 0; i<geomMesh.polygons.size(); i++){
        for(auto &v : geomMesh.polygons[i].idxVertices){
...
#undef get_v
    ...
    // Convert merged mesh's obstacles to draw mesh's obstacles
