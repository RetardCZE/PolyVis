...
std::vector<Point>
SearchInstance::get_point_visibility(Point position, double &T1, double &T2, double &T3) {
    this->vertices.clear();
    clock.Restart();
    std::vector<SearchNodePtr> list = this->getInitNodes(position); // custom version - minor updates
...
void SearchInstance::expand_edge(SearchNodePtr n, Point root, int level){
    //std::cout << *n << std::endl;
    //getchar();
    if(n->next_polygon == -1){
    ...
    int num_succ = get_successors(*n, root, *(this->mesh), search_successors);
    /*
    ...
    */
    SearchNode* nodes = new SearchNode [num_succ];
    const int num_nodes = this->successors2nodes(n, search_successors, num_succ, nodes);
    for(int i = 0; i < num_nodes; i++){
        n = &nodes[i];
...
std::vector<Point> SearchInstance::generate_points(int n){
