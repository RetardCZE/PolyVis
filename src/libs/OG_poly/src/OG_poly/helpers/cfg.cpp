#include "OG_poly/helpers/cfg.h"

OGwarthog::util::cfg::cfg()
{
}
...
void
OGwarthog::util::cfg::parse_args(int argc, char** argv, OGwarthog::util::param params[])
{
    parse_args(argc, argv, "", params);
}

void
OGwarthog::util::cfg::parse_args(int argc, char** argv, const char* options,
...
std::string
OGwarthog::util::cfg::get_param_value(std::string param_name)
{
	std::string ret("");
...
uint32_t
OGwarthog::util::cfg::get_num_values(std::string param_name)
{
    uint32_t ret = 0;
	std::map<std::string, std::vector<std::string>>::iterator it = 
		params_.find(param_name);
...
void
OGwarthog::util::cfg::print(std::ostream& out)
{
	out << "cfg\n";
...
void
OGwarthog::util::cfg::print_values(const std::string& option, std::ostream& out)
{
    std::map<std::string, std::vector<std::string>>::iterator it 
        = params_.find(option);
    if(it != params_.end())
