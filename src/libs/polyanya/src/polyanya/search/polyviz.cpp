...
void PolyVis::expand_edge(SearchNodePtr n, Point root, int level){
    ...
    this->visualise_polygon(next, 2, false, false);
    this->reset_visu();
    this->draw_base(true);
    this->make_frame();
...
PolyVis::get_visibility_polygon(Point position){
    ...
    for(auto n : list){
        initial.push_back(n->right);
        initial.push_back(n->left);
    }
    this->visualise_polygon(initial, 2, false, false);
    seeker = position;
    this->reset_visu();
    this->draw_base(false);
    this->make_frame();
    for(auto n : list){
        currL =n->left;
        currR =n->right;
