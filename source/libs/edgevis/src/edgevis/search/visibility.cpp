...
namespace edgevis{
    ...
    expand(SearchNode node, const Mesh& mesh, std::vector<Point>& visibility, int level, parsers::GeomMesh &gmesh){
        ...
        for(int i = 0; i < num; i++){
            ...
            //getchar();
            expand(nodes[i], mesh, visibility, level+1, gmesh);
        }
        delete [] nodes;
    }

    std::vector<Point>
    find_visibility( int edge, const Mesh& mesh, bool side, parsers::GeomMesh &gmesh){
        int num;
        ...
        for(int i = 0; i < num; i++){
            ...
            expand(nodes[i], mesh, r_vis, 0, gmesh);
        }
        delete [] nodes;
        return r_vis;
    }
